									...
									<c:set var="recno" value="${(docs.currPage-1)*docs.pageSize}" />
										<c:forEach var="doc" items="${voteTasks.lists}">
											...
											<c:set var="recno" value="${recno+1}" />
												...
												<td><a href="${pageContext.request.contextPath}/vote/getVoteQRCodeMobile?voteTaskId=${doc.voteTaskId}" target=_blank>
												   <img style="float:right"  height="60" width="60" alt="扫描二维码" src="${pageContext.request.contextPath}/vote/getVoteQRCodeMobile?voteTaskId=${doc.voteTaskId}">
												</a>
												</td>
												<td>
												<a href="${pageContext.request.contextPath}/vote/voteResult?votetaskid=${doc.voteTaskId}" >投票结果</a></td>
																					<td>
																					<a class="btn btn-default" href="#"
															onclick='deleVote(${doc.voteTaskId})'> <span
...
function goToPage(page){
	...
	document.form1.submit();
		...
		function deleVote(mid){				
			if (confirm("确定删除吗？")) {
				 
				$.ajax({
					type : 'get',
					url : '${pageContext.request.contextPath}/vote/doDeleteVoteTask?voteTaskId='+mid,
